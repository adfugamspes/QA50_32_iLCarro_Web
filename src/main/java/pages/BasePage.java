...
public abstract class BasePage {
    ...
    public <T extends BasePage> T clickButtonHeader(HeaderMenuItem item){
        ...
        switch(item){
            ...
            case SIGN_UP -> {return (T) new RegistrationPage(driver);}
            case SEARCH -> {return (T) new HomePage(driver);}
            case TERMS_OF_USE -> {return (T) new TermsOfUsePage(driver);}
            case LET_THE_CAR_WORK -> {return (T) new LetTheCarWorkPage(driver);}
            case LOGOUT -> {return (T) new HomePage(driver);}
            case DELETE_ACCOUNT -> {return (T) new HomePage(driver);}
